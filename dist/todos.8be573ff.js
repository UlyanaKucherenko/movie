...
var button = document.getElementById('create-todo');
var form = document.getElementById('form');
var todosWraper = document.getElementById('todos');
var todos = [];

var getTodos = function getTodos() {
  var res = JSON.parse(localStorage.getItem('todos'));
  todos = _toConsumableArray(res);
};
...
var renderTodosList = function renderTodosList() {
  todosWraper.innerHTML = todos.map(function (item) {
    return "\n      <div class=\"uk-card uk-card-default uk-card-body uk-margin\">\n      <input  class=\"uk-checkbox todo-input\"   data-id=\"".concat(item.id, "\" type=\"checkbox\" ").concat(item.checked ? "checked" : '', ">\n        <label class=\"uk-h4 todo-label\"> \n          ").concat(item.title, " - ").concat(item.descr, "\n        </label>\n       \n      </div>\n    ");
  }).join("");
};
